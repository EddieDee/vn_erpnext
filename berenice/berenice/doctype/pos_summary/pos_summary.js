...
frappe.ui.form.on('POS Summary', {
    ...
    //     // frm.set_value('pos_user', );
		...
		frm.set_query("usuario", function(doc) {
			return {
				query: "berenice.berenice.doctype.pos_summary.pos_summary.get_cashiers",
				filters: {
					'parent': doc.pos_user
				}
			};
    ...
    if (usuario && posting_date) {
        ...
        // console.log(usuario + fecha);            
        frappe.call({
            method: "berenice.berenice.doctype.pos_summary.pos_summary.get_data",
            args: {
        ...
        frm.clear_table("pos_payments");
            ...
            frappe.call({
            method: "berenice.berenice.doctype.pos_summary.pos_summary.get_mode_of_payment",
            args:{
                // "doc": frm.doc,
                "owner": frm.doc.usuario,
                "posting_date": frm.doc.posting_date,
...
var get_cust_details = function(frm) {
	...
	if (owner && pos_user) {//period_end_date && frm.doc.period_start_date && frm.doc.company && frm.doc.pos_profile && frm.doc.usuario) {
		frappe.call({
			method: "berenice.berenice.doctype.pos_summary.pos_summary.get_cust_details",
			// doc: frm.doc,
			args: {
				owner: owner,
                posting_date: frm.doc.posting_date,
